...
public class GridListView extends Grid implements CommandListener {
    
    private final int MAX_ITEMS = 20;
    private CommandListener parentCommandListener;
    private BackStack backStack;
    
    public GridListView(MIDlet parent, CommandListener commandListener) {
        super(Compatibility.toLowerCaseIfFT("Grid"));
        this.setTheme(CustomList.createTheme(Display.getDisplay(parent)));

        for (int i = 0; i < MAX_ITEMS; i++) {
            this.append(Compatibility.toLowerCaseIfFT("Item " + (i+1)),
                        ImageLoader.loadThumbnail(i % 10));
        }
