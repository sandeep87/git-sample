...
public class FormExample
    ...
    private final String[] TITLES = Compatibility.isFullTouch() ? new String[]{
            "progress + slider",
            "exclusive popup",
            "tumbler",
            "text input",
    ...
    protected void startApp()
        ...
        if (Compatibility.isNonTouch()) {
            // Don't show the Custom example on Non-Touch devices
    ...
    public void commandAction(Command c, Displayable d) {
        if (c == Commands.SELECT) {
            ...
            if (formExampleView != null && d == formExampleView
                && formExampleView.confirmChanges()) {
                // Confirm when backing from the child views which need to save 
                // changes
                confirmBack();
            }
            else {
                // Just go back from the main list view and child views which 
