...
public abstract class UserTask <Params, Progress, Result> {
	...
	/**
	 * Override this method to perform a computation on a background thread. The specified parameters are the parameters passed to {@link #execute(Object[])} by the caller of this task.
	 * 
	 * This method can call {@link #publishProgress(Object[])} to publish updates on the UI thread.
	 * 
	...
	/**
	 * Runs on the UI thread after {@link #publishProgress(Object[])} is invoked. The specified values are the values passed to {@link #publishProgress(Object[])}.
	 * 
	...
	/**
	 * Attempts to cancel execution of this task. This attempt will fail if the task has already completed, already been cancelled, or could not be cancelled for some other reason. If successful, and this task has not started when <tt>cancel</tt> is called, this task should never run. If the task has
	 * already started, then the <tt>mayInterruptIfRunning</tt> parameter determines whether the thread executing this task should be interrupted in an attempt to stop the task.
	 * 
	 ...
	 * 
	 * @throws IllegalStateException If {@link #getStatus()} returns either {@link UserTask.Status#RUNNING} or {@link UserTask.Status#FINISHED}.
	 */
	...
	/**
	 * This method can be invoked from {@link #doInBackground(Object[])} to publish updates on the UI thread while the background computation is still running. Each call to this method will trigger the execution of {@link #onProgressUpdate(Object[])} on the UI thread.
	 * 
