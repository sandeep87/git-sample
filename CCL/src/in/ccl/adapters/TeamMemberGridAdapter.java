...
public class TeamMemberGridAdapter extends BaseAdapter {
	...
	public TeamMemberGridAdapter (Context context, ArrayList <TeamMember> listOfItems, String from) {
		mContext = context;
		isFrom = from;
	...
	public View getView (int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			if (isFrom.equals("team_members")) {
				...
				mViewHolder.imgTeamMember = (PhotoView) convertView.findViewById(R.id.img_team_member);
				...
				if (role.equals(mContext.getResources().getString(R.string.no_role)) || role.equals(mContext.getResources().getString(R.string.ambassadors).toLowerCase())) {
					...
					mViewHolder.txtRole.setText(gridItemsList.get(position).getRole().trim());

				}

				mViewHolder.imgTeamMember.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
				...
				// mViewHolder.image = (ImageView)
				// convertView.findViewById(R.id.image);
				mViewHolder.imgTeamMember = (PhotoView) convertView.findViewById(R.id.img_ambassador);
				mViewHolder.txtPersonName = (TextView) convertView.findViewById(R.id.txt_ambassador_name);
				mViewHolder.errorTxt = (TextView) convertView.findViewById(R.id.error_title);
				// Util.setTextFont((Activity) mContext, mViewHolder.txtRole);
				// Util.setTextFont((Activity) mContext,
				// mViewHolder.txtPersonName);
		...
		mViewHolder.imgTeamMember.setLayoutParams(new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, reqImageHeight));
		mViewHolder.imgTeamMember.setScaleType(ImageView.ScaleType.MATRIX);

		// Util.setTextFont((Activity) mcontext, mViewHolder.title);
		mViewHolder.imgTeamMember.setImageDrawable(mContext.getResources().getDrawable(R.drawable.imagenotqueued));
