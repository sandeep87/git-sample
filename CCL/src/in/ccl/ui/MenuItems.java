...
public class MenuItems implements OnClickListener {
	...
	private TextView liveScoreTxt;

	private AnimationDrawable frameAnimation;

	private AnimationDrawable live_score_frameAnimation;
	...
	public void loadMenu (Activity activity, LinearLayout layout, AnimationLayout mLayout) {
		...
		calenderTxt.setBackgroundResource(R.anim.anim_hot_calender);

		frameAnimation = (AnimationDrawable) calenderTxt.getBackground();
		...
		liveScoreTxt.setBackgroundResource(R.anim.anim_live_score);

		frameAnimation = (AnimationDrawable) liveScoreTxt.getBackground();
		frameAnimation.start();

		// liveScoreTxt.setText("LIVESCORES");
	...
	public void blinkMenuLiveText (Boolean status) {
		if (liveScoreTxt != null) {
			if (status) {
				liveScoreTxt.setBackgroundResource(R.anim.anim_live_score);
				live_score_frameAnimation = (AnimationDrawable) liveScoreTxt.getBackground();
				live_score_frameAnimation.start();
			}
			else {
				// liveScoreTxt.setBackgroundResource(R.anim.anim_live_score);
				// live_score_frameAnimation = (AnimationDrawable) liveScoreTxt.getBackground();
				live_score_frameAnimation.stop();
				liveScoreTxt.setText("LIVESCORES");
				Util.setTextFont(activity, liveScoreTxt);
			}
		}
