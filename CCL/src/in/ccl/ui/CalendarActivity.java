...
public class CalendarActivity extends TopActivity {
	...
	public void onCreate (Bundle savedInstanceState) {
		...
		if (calendarItemsArrayList != null && calendarItemsArrayList.size() > 0) {
			gridviewCalendar.setOnScrollListener(new EndlessScrollListener(CalendarActivity.this, adapter, 0, EndlessScrollListener.RequestType.CALENDAR_IMAGE_REQUEST, calendarItemsArrayList.get(0).getNumberOfPages()));
		}
				...
				case in.ccl.database.Constants.STATE_ACTION_UPDATE_CALENDAR_IMAGE_COMPLETE:
					...
					if (calendarList.size() > 0 && calendarList != null) {
						for (int i = 0; i < calendarList.size(); i++) {
							System.out.println("Calender urls " + calendarList.get(i).getPhotoOrVideoUrl());
						}
