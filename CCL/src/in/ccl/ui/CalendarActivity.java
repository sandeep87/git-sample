...
public class CalendarActivity extends TopActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		txt_calendar_header
				.setText(getResources().getString(R.string.calender));

		if (getIntent().hasExtra(Constants.EXTRA_CALENDAR_KEY)) {
			calendarItemsArrayList = getIntent().getParcelableArrayListExtra(
					Constants.EXTRA_CALENDAR_KEY);
		}

		if (calendarItemsArrayList.size() > 0 && calendarItemsArrayList != null) {
			for (int i = 0; i < calendarItemsArrayList.size(); i++) {
				System.out.println("Calender urls "
						+ calendarItemsArrayList.get(i).getPhotoOrVideoUrl());
			}

			// <<<<<<< Updated upstream
			adapter = new GridAdapter(CalendarActivity.this,
					calendarItemsArrayList, "calendar");
			gridviewCalendar.setAdapter(adapter);
		}
		// =======
		if (calendarItemsArrayList != null && calendarItemsArrayList.size() > 0) {
			gridviewCalendar.setOnScrollListener(new EndlessScrollListener(
					CalendarActivity.this, adapter, 0,
					EndlessScrollListener.RequestType.CALENDAR_IMAGE_REQUEST,
					calendarItemsArrayList.get(0).getNumberOfPages()));
		}
				...
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View view,
						...
						intent.putParcelableArrayListExtra(
								Constants.EXTRA_PHOTO_KEY,
								calendarItemsArrayList);
						intent.putExtra(Constants.EXTRA_PHOTO_POSITION_ID,
								position);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (DownloadManager.ACTION_DOWNLOAD_COMPLETE.equals(action)) {
				Toast.makeText(CalendarActivity.this,
						getResources().getString(R.string.downloading_succuss),
	...
	protected void onResume() {
		...
		LocalBroadcastManager.getInstance(this).registerReceiver(
				mCalenderStateReceiver, statusIntentFilter);
		...
		if (Util.getInstance().isOnline(CalendarActivity.this)) {
			new Handler().postDelayed(new Runnable() {
				...
				@Override
				public void run() {
					Intent mServiceIntent = new Intent(CalendarActivity.this,
							CCLPullService.class).setData(Uri
							.parse(getResources().getString(
									R.string.calender_url)));
	...
	protected void onPause() {
		LocalBroadcastManager.getInstance(CalendarActivity.this)
				.unregisterReceiver(mCalenderStateReceiver);
		...
		public void onReceive(Context context, Intent intent) {
			...
			case in.ccl.database.Constants.STATE_ACTION_UPDATE_CALENDAR_IMAGE_COMPLETE:
				...
				Cursor cursor = getContentResolver().query(
						DataProviderContract.CALENDAR_IMAGE_TABLE_CONTENTURI,
						null, null, null, null);
				CalendarItemsCursor mCalendarItemsCursor = new CalendarItemsCursor();
				ArrayList<Items> calendarList = mCalendarItemsCursor
						.getItems(cursor);
				if (cursor != null) {
					cursor.close();
				}
				if (calendarList.size() > 0 && calendarList != null) {
					for (int i = 0; i < calendarList.size(); i++) {
						System.out.println("Calender urls "
								+ calendarList.get(i).getPhotoOrVideoUrl());
					}
					adapter = new GridAdapter(CalendarActivity.this,
