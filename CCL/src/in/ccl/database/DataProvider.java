...
/**
 * 
 * Defines a ContentProvider that stores URLs of CCL featured pictures The provider also has a table that tracks the last time a picture URL was updated.
 */
public class DataProvider extends ContentProvider {

	// Indicates that the incoming query is for a picture URL
	public static final int BANNER_IMAGE_URL_QUERY = 1;
	...
	public static final int PHOTO_ALBUM_URL_QUERY = 3;

	public static final int VIDEO_ALBUM_URL_QUERY = 4;

	public static final int PHOTO_URL_QUERY = 5;

	public static final int VIDEO_URL_QUERY = 6;

	public static final int PAGES_QUERY = 7;
	...
	private static final String CREATE_VIDEO_ALBUM_TABLE_SQL = "CREATE TABLE" + " " + DataProviderContract.VIDEO_ALBUM_TABLE_NAME + " " + "(" + " " + DataProviderContract.ROW_ID + " " + PRIMARY_KEY_TYPE + " ," + DataProviderContract.VIDEO_ALBUM_IMAGE_URL_COLUMN + " " + TEXT_TYPE + " ," + DataProviderContract.IMAGE_NAME_COLUMN + " " + TEXT_TYPE + ");";
	...
	private static final String CREATE_TEAM_LOGO_TABLE_SQL = "CREATE TABLE" + " " + DataProviderContract.TEAMS_LOGO_TABLE_NAME + " " + "(" + " " + DataProviderContract.TEAM_ID_COLUMN + " " + PRIMARY_KEY_TYPE + " ," + DataProviderContract.TEAM_NAME_COLUMN + " " + TEXT_TYPE + " ," + DataProviderContract.TEAM_LOGO_IMAGE_URL_COLUMN + " " + TEXT_TYPE + " ," + DataProviderContract.TEAM_BANNER_IMAGE_URL_COLUMN + " " + TEXT_TYPE + ");";
	...
	/*
	 * Initializes meta-data used by the content provider: - UriMatcher that maps content URIs to codes - MimeType array that returns the custom MIME type of a table
	 */
	static {
		...
		/*
		 * Sets up an array that maps content URIs to MIME types, via a mapping between the URIs and an integer code. These are custom MIME types that apply to tables and rows in this particular provider.
		 */
		sMimeTypes = new SparseArray <String>();

		// Adds a URI "match" entry that maps picture URL content URIs to a numeric code
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.BANNERURL_TABLE_NAME, BANNER_IMAGE_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.PHOTO_ALBUM_TABLE_NAME, PHOTO_ALBUM_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.VIDEO_ALBUM_TABLE_NAME, VIDEO_ALBUM_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.RAW_TABLE_NAME + "/#", PHOTO_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.RAW_TABLE_NAME + "/#", VIDEO_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.RAW_TABLE_NAME, PHOTO_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.RAW_TABLE_NAME, VIDEO_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.DOWNLOAD_IMAGE_TABLE_NAME, DOWNLOAD_IMAGE_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.TEAMS_LOGO_TABLE_NAME, TEAM_LOGO_URL_QUERY);
		sUriMatcher.addURI(DataProviderContract.AUTHORITY, DataProviderContract.TEAM_MEMBERS_TABLE_NAME, TEAM_MEMBERS_URL_QUERY);
		...
		sMimeTypes.put(PHOTO_ALBUM_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.PHOTO_ALBUM_TABLE_NAME);
		sMimeTypes.put(VIDEO_ALBUM_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.VIDEO_ALBUM_TABLE_NAME);
		sMimeTypes.put(PHOTO_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.RAW_TABLE_NAME);
		sMimeTypes.put(VIDEO_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.RAW_TABLE_NAME);
		sMimeTypes.put(PAGES_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.PAGES_TABLE_NAME);
		sMimeTypes.put(CATEGORY_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.CATEGORY_TABLE_NAME);
		sMimeTypes.put(DOWNLOAD_IMAGE_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.DOWNLOAD_IMAGE_TABLE_NAME);
		sMimeTypes.put(TEAM_LOGO_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.TEAMS_LOGO_TABLE_NAME);
		sMimeTypes.put(TEAM_MEMBERS_URL_QUERY, "vnd.android.cursor.dir/vnd." + DataProviderContract.AUTHORITY + "." + DataProviderContract.TEAM_MEMBERS_TABLE_NAME);
	...
	/**
	 * Defines a helper class that opens the SQLite database for this provider when a request is received. If the database doesn't yet exist, the helper creates it.
	 */
	private class DataProviderHelper extends SQLiteOpenHelper {
		...
		private void dropTables (SQLiteDatabase db) {
			...
			db.execSQL("DROP TABLE IF EXISTS " + DataProviderContract.BANNERURL_TABLE_NAME);
			db.execSQL("DROP TABLE IF EXISTS " + DataProviderContract.PHOTO_ALBUM_TABLE_NAME);
			db.execSQL("DROP TABLE IF EXISTS " + DataProviderContract.VIDEO_ALBUM_TABLE_NAME);
			db.execSQL("DROP TABLE IF EXISTS " + DataProviderContract.RAW_TABLE_NAME);
		...
		/**
		 * Does setup of the database. The system automatically invokes this method when SQLiteDatabase.getWriteableDatabase() or SQLiteDatabase.getReadableDatabase() are invoked and no db instance is available.
		 * 
		...
		public void onCreate (SQLiteDatabase db) {
			...
			db.execSQL(CREATE_BANNER_TABLE_SQL);
			db.execSQL(CREATE_PHOTO_ALBUM_TABLE_SQL);
			db.execSQL(CREATE_VIDEO_ALBUM_TABLE_SQL);
		...
		/**
		 * Handles upgrading the database from a previous version. Drops the old tables and creates new ones.
		 * 
		...
		/**
		 * Handles downgrading the database from a new to a previous version. Drops the old tables and creates new ones.
		 * 
	...
	/**
	 * Initializes the content provider. Notice that this method simply creates a the SQLiteOpenHelper instance and returns. You should do most of the initialization of a content provider in its static initialization block or in SQLiteDatabase.onCreate().
	 */
	@Override
	...
	/**
	 * Implements bulk row insertion using {@link SQLiteDatabase#insert(String, String, ContentValues) SQLiteDatabase.insert()} and SQLite transactions. The method also notifies the current {@link android.content.ContentResolver} that the {@link android.content.ContentProvider} has been changed.
	 * 
	 * @see android.content.ContentProvider#bulkInsert(Uri, ContentValues[])
	 * @param uri The content URI for the insertion
	 * @param insertValuesArray A {@link android.content.ContentValues} array containing the row to insert
	...
	public int bulkInsert (Uri uri, ContentValues[] insertValuesArray) {
		...
		switch (sUriMatcher.match(uri)) {
			...
			case BANNER_IMAGE_URL_QUERY:
				...
				// localSQLiteDatabase.delete(DataProviderContract.BANNERURL_TABLE_NAME, null, null);
			...
			case VIDEO_ALBUM_URL_QUERY:
				...
				// Deletes all the existing rows in the table
				// localSQLiteDatabase.delete(DataProviderContract.VIDEO_ALBUM_TABLE_NAME, null, null);
         ...
         int numberOfRowsUpdated = 0;
				...
				// Deletes all the existing rows in the table
				// localSQLiteDatabase.delete(DataProviderContract.RAW_TABLE_NAME, null, null);
			...
			case PAGES_QUERY:
				...
				// Deletes all the existing rows in the table
				// localSQLiteDatabase.delete(DataProviderContract.PAGES_TABLE_NAME, null, null);
         ...
         count = 0;
				...
				// Deletes all the existing rows in the table
				localSQLiteDatabase.delete(DataProviderContract.CATEGORY_TABLE_NAME, null, null);
	 ...
	 * @param selection The SQL WHERE string. Use "?" to mark places that should be substituted by values in selectionArgs.
	 * @param selectionArgs An array of values that are mapped to each "?" in selection. If no "?" are used, set this to NULL.
	 * 
	 ...
	 * @param values The values to use to update the row or rows. You only need to specify column names for the columns you want to change. To clear the contents of a column, specify the column name and NULL for its value.
	 * @param selection An SQL WHERE clause (without the WHERE keyword) specifying the rows to update. Use "?" to mark places that should be substituted by values in selectionArgs.
	 * @param selectionArgs An array of values that are mapped in order to each "?" in selection. If no "?" are used, set this to NULL.
	 * 
