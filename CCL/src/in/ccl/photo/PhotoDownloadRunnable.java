 ...
 * 
 * Objects of this class are instantiated and managed by instances of PhotoTask, which implements the methods of {@link TaskRunnableDecodeMethods}. PhotoTask objects call {@link #PhotoDownloadRunnable(TaskRunnableDownloadMethods) PhotoDownloadRunnable()} with themselves as the argument. In effect,
 * an PhotoTask object and a PhotoDownloadRunnable object communicate through the fields of the PhotoTask.
 */
class PhotoDownloadRunnable implements Runnable {
	...
	/**
	 * 
	 * An interface that defines methods that PhotoTask implements. An instance of PhotoTask passes itself to an PhotoDownloadRunnable instance through the PhotoDownloadRunnable constructor, after which the two instances can access each other's variables.
	 */
	...
	/**
	 * This constructor creates an instance of PhotoDownloadRunnable and stores in it a reference to the PhotoTask instance that instantiated it.
	 * 
	...
	public void run () {
		...
		/*
		 * Stores the current Thread in the the PhotoTask instance, so that the instance can interrupt the Thread.
		 */
		...
		/*
		 * Gets the image cache buffer object from the PhotoTask instance. This makes the to both PhotoDownloadRunnable and PhotoTask.
		 */
		byte[] byteBuffer = mPhotoTask.getByteBuffer();

		/*
		 * A try block that downloads a Picasa image from a URL. The URL value is in the field PhotoTask.mImageURL
		 */
		...
		try {
			...
			if (null == byteBuffer) {
				...
				/*
				 * Calls the PhotoTask implementation of {@link #handleDownloadState} to set the state of the download
				 */
				...
				try {
					...
					if (-1 == contentSize) {
						...
						/*
						 * Defines the initial offset of the next available byte in the buffer, and the initial result of reading the binary
						 */
						int bufferOffset = 0;
						int readResult = 0;

						/*
						 * The "outer" loop continues until all the bytes have been downloaded. The inner loop continues until the temporary buffer is full, and then allocates more buffer space.
						 */
						outer :
						do {
							while (bufferLeft > 0) {
								...
								/*
								 * Reads from the URL location into the temporary buffer, starting at the next available free byte and reading as many bytes as are available in the buffer.
								 */
								...
								/*
								 * The read isn't finished. This sets the next available open position in the buffer (the buffer index is 0-based).
								 */
							...
							/*
							 * The temporary buffer is full, so the following code creates a new buffer that can contain the existing contents plus the next read cycle.
							 */
							...
							/*
							 * Sets a new size that can contain the existing buffer's contents plus space for the next read cycle.
							 */
							int newSize = tempBuffer.length + READ_SIZE;

							/*
							 * Creates a new temporary buffer, moves the contents of the old temporary buffer into it, and then points the temporary buffer variable to the new buffer.
							 */
						...
						/*
						 * When the entire image has been read, this creates a permanent byte buffer with the same size as the number of used bytes in the temporary buffer (equal to the next open byte, because tempBuffer is 0=based).
						 */
