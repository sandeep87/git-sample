...
import in.ccl.photo.PhotoDownloadRunnable.TaskRunnableDownloadMethods;
 ...
 * itself as an argument to the constructor of a download or decode object. In effect, this allows PhotoTask to start on a Thread, run a download in a delegate object, then run a decode, and then start over again. This class can be pooled and reused as necessary.
 */
public class PhotoTask implements TaskRunnableDownloadMethods, TaskRunnableDecodeMethods {

	/*
	 * Creates a weak reference to the ImageView that this Task will populate. The weak reference prevents memory leaks and crashes, because it automatically tracks the "state" of the variable it backs. If the reference becomes invalid, the weak reference is garbage- collected. This technique is
	 * important for referring to objects that are part of a component lifecycle. Using a hard reference may cause memory leaks as the value continues to change; even worse, it can cause crashes if the underlying component is destroyed. Using a weak reference to a View ensures that the reference is
	...
	/*
	 * Fields containing references to the two runnable objects that handle downloading and decoding of the image.
	 */
	...
	/*
	 * Sets the identifier for the current Thread. This must be a synchronized operation; see the notes for getCurrentThread()
	 */
	...
	/*
	 * Implements PhotoDownloadRunnable.handleHTTPState(). Passes the download state to the ThreadPool object.
	 */
	...
	/*
	 * Implements PhotoDecodeRunnable.handleDecodeState(). Passes the decoding state to the ThreadPool object.
	 */
	@Override
