...
/**
 * This class creates pools of background threads for downloading Picasa images from the web, based on URLs retrieved from Picasa's featured images RSS feed. The class is implemented as a singleton; the only way to get an PhotoManager instance is to call {@link #getInstance}.
 * <p>
 * The class sets the pool size and cache size based on the particular operation it's performing. The algorithm doesn't apply to all situations, so if you re-use the code to implement a pool of threads for your own app, you will have to come up with your choices for pool size, cache size, and so
 * forth. In many cases, you'll have to set some numbers arbitrarily and then measure the impact on performance.
 * <p>
 * This class actually uses two threadpools in order to limit the number of simultaneous image decoding threads to the number of available processor cores.
 * <p>
 * Finally, this class defines a handler that communicates back to the UI thread to change the bitmap to reflect the state.
 */
...
public class PhotoManager {
	...
	/**
	 * NOTE: This is the number of total available cores. On current versions of Android, with devices that use plug-and-play cores, this will return less than the total number of cores. The total number of cores is not available in current Android implementations.
	 */
	private static int NUMBER_OF_CORES = Runtime.getRuntime().availableProcessors();

	/*
	 * Creates a cache of byte arrays indexed by image URLs. As new items are added to the cache, the oldest items are ejected and subject to garbage collection.
	 */
	...
	private PhotoView localView;

	private ScaleImageView scalelocalView;
	...
	private PhotoManager () {
		...
		/*
		 * Creates a work queue for the pool of Thread objects used for downloading, using a linked list queue that blocks when the queue is empty.
		 */
		...
		/*
		 * Instantiates a new anonymous Handler object and defines its handleMessage() method. The Handler *must* run on the UI thread, because it moves photo Bitmaps from the PhotoTask object to the View object. To force the Handler to run on the UI thread, it's defined as part of the PhotoManager
		 * constructor. The constructor is invoked when the class is first referenced, and that happens when the View invokes startDownload. Since the View runs on the UI Thread, so does the constructor and the Handler.
		 */
		mHandler = new Handler(Looper.getMainLooper()) {
			...
			public void handleMessage (Message inputMessage) {
				...
				// input ImageView
				boolean flag = photoTask.isViewZoom();
				if (!flag) {
				...
				if (localView != null || scalelocalView != null) {
					...
					/*
					 * Gets the URL of the *weak reference* to the input ImageView. The weak reference won't have changed, even if the input ImageView has.
					 */
					URL localURL = null;
					if (!flag && localView != null) {
						localURL = localView.getLocation();
					}
					else if (scalelocalView != null) {
						localURL = scalelocalView.getLocation();

					}
					/*
					 * Compares the URL of the input ImageView to the URL of the weak reference. Only updates the bitmap in the ImageView if this particular Thread is supposed to be serving the ImageView.
					 */
						...
						switch (inputMessage.what) {
							...
							case DOWNLOAD_STARTED:
								if (!flag && localView != null) {
									...
									scalelocalView.setStatusResource(DOWNLOAD_STARTED);
								}
								break;

							/*
							 * If the download is complete, but the decode is waiting, sets the background color to golden yellow
							 */
							...
							case DECODE_STARTED:
								...
								break;
							/*
							 * The decoding is done, so this sets the ImageView's bitmap to the bitmap in the incoming message
							 */
							case TASK_COMPLETE:
	...
	public void handleState (PhotoTask photoTask, int state) {
		switch (state) {
			...
			case DOWNLOAD_COMPLETE:
				/*
				 * Decodes the image, by queuing the decoder object to run in the decoder thread pool
				 */
	...
	public static void cancelAll () {
		...
		/*
		 * Locks on the singleton to ensure that other processes aren't mutating Threads, then iterates over the array of tasks and interrupts the task's current Thread.
		 */
	...
	static public void removeDownload (PhotoTask downloaderTask, URL pictureURL) {
		...
		if (downloaderTask != null && downloaderTask.getImageURL().equals(pictureURL)) {
			...
			/*
			 * Removes the download Runnable from the ThreadPool. This opens a Thread in the ThreadPool's work queue, allowing a task in the queue to start.
			 */
	 ...
	 */
	static public PhotoTask startDownload (ImageView imageView, boolean cacheFlag, boolean flag) {
		...
		downloadTask.initializeDownloaderTask(PhotoManager.sInstance, imageView, true, flag);

		/*
		 * Provides the download task with the cache buffer corresponding to the URL to be downloaded.
		 */
		...
		if (null == downloadTask.getByteBuffer()) {
			...
			/*
			 * "Executes" the tasks' download Runnable in order to download the image. If no Threads are available in the thread pool, the Runnable waits in the queue.
			 */
			...
			if (!flag) {
				((PhotoView) imageView).setStatusResource(R.drawable.imagequeued);
			}
			else {
				((ScaleImageView) imageView).setStatusResource(R.drawable.imagequeued);
			}
			...
			/*
			 * Signals that the download is "complete", because the byte array already contains the undecoded image. The decoding starts.
			 */
