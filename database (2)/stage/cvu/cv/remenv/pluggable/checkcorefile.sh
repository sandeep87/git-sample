#!/bin/sh
#
# $Header: opsm/cvutl/pluggable/checkcorefile.sh /main/3 2010/06/13 13:42:28 kfgriffi Exp $
#
# checkcorefile.sh
#
...
case $PLATFORM in
  SunOS | HP-UX)
      ...
      then
        result="<RESULT>SUCC</RESULT><TRACE>Core File Dump feature is enabled on node $host</TRACE><NLS_MSG><FACILITY>Prve</FACILITY><ID>0032</ID><MSG_DATA><DATA>$host</DATA></MSG_DATA></NLS_MSG>"
        existstatus=0
      else
        result="<RESULT>WARN</RESULT><TRACE>Core File Dump feature is NOT enabled on node $host</TRACE><NLS_MSG><FACILITY>Prve</FACILITY><ID>0033</ID><MSG_DATA><DATA>$host</DATA></MSG_DATA></NLS_MSG>"
        existstatus=2
      fi
      ;;
  AIX)
