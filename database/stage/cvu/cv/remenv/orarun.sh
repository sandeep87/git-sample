#!/bin/sh
#
# $Header: oui/prov/fixup/linuxS390/orarun.sh /main/4 2009/12/02 22:17:40 sandgoya Exp $
#
# orarun.sh
#
...
#
#    MODIFIED   (MM/DD/YY)
#     sandgoya 12/02/09 - bugfix 8611786
#     sandgoya 11/18/09 - bugfix 8689883
...
then
	...
	if [ -n "$SEMMSL" -o -n "$SEMMNS" -o -n "$SEMOPM" -o -n "$SEMMNI" ]
	  ...
	  if [ ! -z "$SEMMNI" ]
          ...
          then
             ...
             # if any value is not set in /etc/sysctl.conf file then pick value from  /proc/sys bugfix 8689883
                 if [ -z $file_semmsl ]
                    then
                        file_semmsl=$cur_semmsl
                fi
                if [ -z $file_semmns ]
                    then
                        file_semmns=$cur_semmns
                fi
                if [ -z $file_semopm ]
                    then
                        file_semopm=$cur_semopm
                fi
                if [ -z $file_semmni ]
                    then
                        file_semmni=$cur_semmni
                fi
	...
	#FILE_MAX_KERNEL, IP_LOCAL_PORT_RANGE, RMEM_DEFAULT, WMEM_DEFAULT, RMEM_MAX, WMEM_MAX,AIO_MAX_SIZE
	   ...
	   then
	      ...
	      echo "The upper limit of ip_local_port range in response file is not greater than value in /etc/sysctl.conf, hence not changing it."|tee -a $log_file/orarun.log
	   fi
           if [ $flag == "true" ]
           then
