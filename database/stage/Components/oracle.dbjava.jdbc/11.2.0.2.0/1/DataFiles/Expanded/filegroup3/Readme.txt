...
Lob data-in-locator 
    Starting with database 10.2 the data may be stored in the locator
    itself for small lobs which are stored in the row in the database.
    This occurs only if the data is  3898 bytes or less. Reading this
    data is supported by the jdbc driver only for database version 10.2.
    For database versions starting with 11.1 use lob prefetch.  
...
Lob prefetch
    ...
    LOBS. Supportted for database version starting with 11.1. Lob
    prefetch supplants data-in-locator and disables it when used.
...
BUG-9467659
    Error code ORA-17360 for "Invalid LOB prefetch size" changed to 
    ORA-17267 and ORA-17361 "Year out of range" changed to ORA-17268
    to conform to numbering conventions. These codes were added in
    11.2.0.1
